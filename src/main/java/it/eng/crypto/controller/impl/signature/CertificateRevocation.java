...
/**
 * Prende in considerazione come riferimento temporale il timestamp in input (se presente) oppure la data attuale.
 * Verifica la validità dei certificati di firma rispetto
 * <ol>
 * <li>Alle CRL recuperate dalla firma digitale (se presenti)</li>
 * <li>Alla CRL scaricata dal distribution point indicato nella busta – il controllo prende in considerazione solamente
 * le CRL valide al riferimento temporale, presenti tra gli attributi firmati di ciascuna firma</li>
...
public class CertificateRevocation extends AbstractSignerController {
    ...
    public boolean execute(InputSignerBean input, OutputSignerBean output) throws ExceptionController {
        ...
        if (output.getProperties().containsKey(OutputSignerBean.SIGNATURE_PROPERTY)) {
            ...
            result = populateValidationInfosMapFromInputOutput(validationInfosMap, crlInfosMap, input, output,
                    signatures);
    ...
    private boolean populateValidationInfosMapFromInputOutput(Map<ISignature, ValidationInfos> validationInfosMap,
        ...
        for (ISignature signature : signatures) {
            ...
            /*
             * Verifico se il certificato era già scaduto alla data di riferimento temporale Solo se la configurazione
             * prevede questa verifica
             */
            ...
            if (certificateExpirationInfo != null && !certificateExpirationInfo.isValid()) {
                if (certificateExpirationInfo.getEsito().equals(EsitoControllo.CERTIFICATO_ERRATO)) {
                    validationInfos.addError("Certificato di firma errato, verifica CRL non effettuata");
                } else {
                ...
                // TODO MQ: verificare se aggiungere questo controllo anche durante il CertificateReliability
                // Verifico se la data di prossimo aggiornamento della CRL è >= della
                // data del riferimento temporale
                if (historicalCRL != null && historicalCRL.getNextUpdate().after(signature.getReferenceDate())) {
                    ...
                    try {
                        ...
                        if (urlCRLDistributionPoints != null) {
                            ...
                            if (distributionPointCRL != null
                                    && distributionPointCRL.getNextUpdate().after(signature.getReferenceDate())) {
                                log.debug("effettuo il controllo di revoca");
                                checkCRL(validationInfos, signature, distributionPointCRL, crlInfosMap);
                            } else {
                                validationInfos
                                        .addError("La CRL ottenuta dal punto di distribuzione dalla CA scade il: "
                                                + dateFormatter.format(distributionPointCRL.getNextUpdate())
                                ...
                                // anche in caso di CRL_SCADUTA devo popolare la mappa: la CRL deve essere persistita
                        ...
                        // TODO controllo se la mappa è diversa da null ... potrei aver non effettuato il controllo di
                            ...
                            try {
                                ...
                                if (embeddedCRLs != null && embeddedCRLs.size() != 0) {
                                        ...
                                        if (embeddedCRL instanceof X509CRL) {
                                            ...
                                            try {
                                                ...
                                                if (x509EmbeddedCRL.getNextUpdate()
                                                    ...
                                                    checkCRL(validationInfos, signature, (X509CRL) embeddedCRL,
                                                            crlInfosMap);
                                    ...
                                    if (!existsEmbeddedCRLReferredToIssuer) {
                                        validationInfos.addWarning("Nella busta sono presenti " + embeddedCRLs.size()
                                ...
                                if (embeddedCRLs == null || embeddedCRLs.size() == 0
                                    ...
                                    if (inputCRL instanceof X509CRL) {
                                        ...
                                        try {
                                            ...
                                            if (x509InputCRL.getNextUpdate().after(signature.getReferenceDate())) {
                                                checkCRL(validationInfos, signature, x509InputCRL, crlInfosMap);
                                        ...
                                        // Se il certificato della CA è scaduto prima del 3 dicembre 2009 la CRL
            ...
            if (date != null && crlEntry.getRevocationDate().before(date)) {
                validationInfos
                        .addError("Certificato revocato in data: " + dateFormatter.format(crlEntry.getRevocationDate())
                                + " (antecedente a: " + dateFormatter.format(date) + ")" + reason);
