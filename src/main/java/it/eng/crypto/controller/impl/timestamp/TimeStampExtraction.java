...
public class TimeStampExtraction extends AbstractTimeStampController {
    ...
    public boolean execute(InputTimeStampBean input, OutputTimeStampBean output)
        ...
        try {
            ...
            if (input.getTimeStampWithContentFile() != null) {
                ...
                if (timeStampTokens != null) {
                    for (TimeStampToken timeStampToken : timeStampTokens) {
                        documentAndTimeStampInfoList.add(addDocumentAndTSInfoBeanDetached(
                                input.getContentFile(), timeStampToken, signer));
                    }
                }
                try {
                    ...
                    // se il file embedded non è firmato la signerUtil.getSignerManager() lancia
                    // un'eccezione che
    ...
    private DocumentAndTimeStampInfoBean addDocumentAndTSInfoBeanEmbedded(File inputFile,
            TimeStampToken timeStampToken, AbstractSigner signer)
        ...
        /*
         * Tipo di marca (EMBEDDED) oppure EMBEDDED M7M (che di fatto è una detached in append su un
         * p7m)
         */
        if (signer.getFormat().equals(SignerType.M7M)) {
            documentAndTimeStampInfo.setTimeStampTokenType(
                    DocumentAndTimeStampInfoBean.TimeStampTokenType.EMBEDDED_M7M);
        } else if (signer.getFormat().equals(SignerType.TSD)) {
            documentAndTimeStampInfo.setTimeStampTokenType(
                    DocumentAndTimeStampInfoBean.TimeStampTokenType.EMBEDDED_TSD);
        } else {
            documentAndTimeStampInfo.setTimeStampTokenType(
    ...
    private DocumentAndTimeStampInfoBean addDocumentAndTSInfoBeanDetached(File inputFile,
            TimeStampToken timeStampToken, AbstractSigner signer)
            throws CloneNotSupportedException {
        DocumentAndTimeStampInfoBean documentAndTimeStampInfo = new DocumentAndTimeStampInfoBean();
