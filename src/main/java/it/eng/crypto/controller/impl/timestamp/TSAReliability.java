...
public class TSAReliability extends AbstractTimeStampController {
    ...
    public boolean execute(InputTimeStampBean input, OutputTimeStampBean output) throws ExceptionController {
        ...
        for (DocumentAndTimeStampInfoBean documentAndTimeStampInfo : documentAndTimeStampInfos) {
                    ...
                    if (saCertificate instanceof X509Certificate) {
                        ...
                        if (saX509Certificate.getSerialNumber().equals(tsaSerial)) {
                            ...
                            if (qualifiedCertificate == null) {
                                ...
                                X509Certificate issuerCertificate = certificatesAuthorityStorage.retriveCA(
                                        saX509Certificate.getIssuerX500Principal(),
                                        signerUtil.getAuthorityKeyId(saX509Certificate));

                                if (input.isCheckCAOnline()) {
                                    // Potrebbe succedere che il DB delle CA (TSA) non sia ancora stato riempito. Provo
                                    // a
                                    // controllare ONLINE.
                                    if (issuerCertificate == null) {
                                        X509Certificate tsaCert = signerUtil.getCACertificateOnline(saX509Certificate,
                                                certificatesAuthorityStorage);
                                        if (tsaCert != null) {
                                            issuerCertificate = tsaCert;
                                            try {
                                                // per motivi di performance aggiungo subito la CA rischiando contesa
                                                // con i
                                                // job.
                                ...
                                if (issuerCertificate == null) {
                                    ...
                                    // MODIFICATO PER IL PARER!! LA MARCA E' VALIDA (o meglio può essere usata come
                                    // riferimento
                                    // temporale) ANCHE SE LA TSA NON E' ACCREDITATA
                                    validationInfos.addWarning("Il certificato della TSA non è accreditato");
                                    catenaValidationInfos.addError("Il certificato della TSA non è accreditato");
                                    ...
                                    try {
                                        ...
                                        // MODIFICATO PER IL PARER!! LA MARCA E' VALIDA ANCHE SE LA TSA NON E'
                                        // ACCREDITATA
                            ...
                            if (qualifiedCertificate != null) {
                                ...
                                if (org.bouncycastle.util.Arrays.constantTimeAreEqual(
                                        saCertificate.getPublicKey().getEncoded(), publicKey.getEncoded())) {
                                    /*
                                    ...
                                    if (timestampDate.after(qualifiedCertificate.getNotAfter())) {
                                        validationInfos.addError(
                                                "La data di generazione della marca temporale: " + timestampDate
                                        ...
                                        certificatoValidationInfos.addError(
                                                "La data di generazione della marca temporale: " + timestampDate
                                                        + " è successiva alla data di scadenza del certificato: "
                                                        + qualifiedCertificate.getNotAfter());
                                        certificatoValidationInfos.setEsito(EsitoControllo.NEGATIVO);
                                        isCertificateInList = false;
                                        result = false;
                            ...
                            documentAndTimeStampInfo.setProperty(DocumentAndTimeStampInfoBean.PROP_CERTIFICATE,
                                    qualifiedCertificate == null ? saCertificate : qualifiedCertificate);
            ...
            documentAndTimeStampInfo.setProperty(TipoControlli.CATENA_TRUSTED.name(), catenaValidationInfos);
            documentAndTimeStampInfo.setProperty(TipoControlli.CERTIFICATO.name(), certificatoValidationInfos);
