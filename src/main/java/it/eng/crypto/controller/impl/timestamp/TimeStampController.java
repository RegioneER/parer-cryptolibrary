...
public class TimeStampController implements ITimeStampController {
    ...
    private void populateCommonAttributes(DocumentAndTimeStampInfoBean documentAndTimeStampInfo,
                        ...
                        if (qualifiedCertificate == null) {
                            ...
                            if (org.bouncycastle.util.Arrays.constantTimeAreEqual(
                                ...
                                if (timestampDate.after(qualifiedCertificate.getNotAfter())) {
                                    validationInfos.addError("La data di generazione della marca temporale: "
                                            + timestampDate + " è successiva alla data di scadenza del certificato: "
                                            + qualifiedCertificate.getNotAfter());
                                    isCertificateInList = false;
                                } else if (timestampDate.before(qualifiedCertificate.getNotBefore())) {
                                    validationInfos
                                            .addError("La data di generazione della marca temporale: " + timestampDate
                                                        ...
                                                        try {
                                                            ...
                                                            if (x509EmbeddedCRL.getNextUpdate().after(timestampDate)) {
                                                                ...
                                                                checkCRL(validationInfos, saX509Certificate,
                                                                        (X509CRL) embeddedCRL, timestampDate);

                                                                // Tengo traccia che almeno una CRL nella busta è
                                                                // relativa
                                                            ...
                                                            // Una CRL nella busta non è relativa al certificato
                                                            // dell'issuer
        ...
        documentAndTimeStampInfo.setProperty(DocumentAndTimeStampInfoBean.PROP_RECOGNIZED_CERTIFICATE,
        ...
        if (timeStampTokens != null) {
            for (TimeStampToken timeStampToken : timeStampTokens) {
                ...
                documentAndTimeStampInfo
                        .setTimeStampTokenType(DocumentAndTimeStampInfoBean.TimeStampTokenType.EMBEDDED);
    ...
    /**
     * @see it.eng.crypto.controller.impl.timestamp.TimeStampController#checkTimeStamps(File, boolean)
     *      checkTimeStamps(File,boolean)
     */
    ...
    private DocumentAndTimeStampInfoBean[] checkTimeStamps(File file, File detachedTimeStamp,
        ...
        if (timeStampTokens != null) {
            for (TimeStampToken timeStampToken : timeStampTokens) {
                 ...
                 */
                documentAndTimeStampInfo
                        .setTimeStampTokenType(DocumentAndTimeStampInfoBean.TimeStampTokenType.DETACHED);
                ...
                if (infos == null) {
                    infos = new ValidationInfos();
                    infos.addWarning("Il signer non ha effettuato la validazione oppure la marca non è di tipo: "
                            + documentAndTimeStampInfo.getTimeStampTokenType());
                }
    ...
    /**
     * @see it.eng.crypto.controller.impl.timestamp.TimeStampController#checkTimeStamps(File, File, boolean)
     *      checkTimeStamps(File, File, boolean)
     */
    ...
    private void checkCRL(ValidationInfos validationInfos, X509Certificate signatureCertificate, X509CRL crl,
            Date date) {
    ...
    /**
     * Recupera l'istanza del validatore {@link it.eng.crypto.controller.ITimeStampValidator} preposto alla verifica
     * della validità corrente delle marche temporali
     */
    ...
    /**
     * Definisce l'istanza del validatore {@link it.eng.crypto.controller.ITimeStampValidator} preposto alla verifica
     * della validità corrente delle marche temporali
     */
    ...
    private void validateTimeStampsChain(DocumentAndTimeStampInfoBean[] documentAndTimeStampInfos, File signedFile,
            ...
            if (extensionSigner != null) {
                ...
                if (timeStampTokenExtensionValidation == null || !timeStampTokenExtensionValidation.isValid()) {
                    setAllValidationInfos(documentAndTimeStampInfos,
                            new String[] { "L'estensione della marca temporale #" + (i + 1) + " contenuta nel file: "
                                    + timeStampExtensionFile + " è invalida per il file: " + currentFile },
                            null);
                    break;
                }
                ...
                if (timeStampExtensions != null && timeStampExtensions.length != 0) {
                    ...
                    if (!validateTimeStampExtensionListOverTimeStampList(documentAndTimeStampInfos,
                            currentTimeStampTokens, timeStampExtensions, timeStampExtensionFile, currentFile))
                        break;

                    // Valido la TSA dell'estensione
                    validateTimeStampExtensionListTSA(documentAndTimeStampInfos, timeStampExtensions,
    ...
    private void validateTimeStampExtensionListTSA(DocumentAndTimeStampInfoBean[] documentAndTimeStampInfos,
        ...
        for (int i = 0; i < timeStampExtensions.length; ++i) {
            ...
            if (tmpValidationInfos.getWarnings() != null && tmpValidationInfos.getWarnings().length != 0) {
                setAllValidationInfos(documentAndTimeStampInfos, null,
                        new String[] { "L'estensione della marca temporale #" + (i + 1) + " contenuta nel file: "
                                + timeStampExtensionFile + " contiene un certificato con il seguente problema: "
    ...
    private boolean validateLastTimeStampExtensions(DocumentAndTimeStampInfoBean[] documentAndTimeStampInfos,
            TimeStampToken[] timeStampExtensions, File timeStampExtensionFile) {
        boolean result = true;
        for (int i = 0; i < timeStampExtensions.length; ++i) {
            TimeStampToken timeStampExtension = timeStampExtensions[i];
            if (!timeStampValidator.isTimeStampCurrentlyValid(timeStampExtension,
                ...
                setAllValidationInfos(
                        documentAndTimeStampInfos, new String[] { "L'estensione della marca temporale #" + (i + 1)
                                + " contenuta nel file: " + timeStampExtensionFile + " non è correntemente valida" },
                        null);
                result = false;
            }
        }
    ...
    private boolean validateTimeStampExtensionListOverTimeStampList(
            DocumentAndTimeStampInfoBean[] documentAndTimeStampInfos, TimeStampToken[] currentTimeStampTokens,
            TimeStampToken[] timeStampExtensions, File timeStampExtensionFile, File currentFile) {
        boolean result = true;
        for (int i = 0; i < currentTimeStampTokens.length; i++) {
            ...
            for (int j = 0; j < timeStampExtensions.length; j++) {
                ...
                if (!timeStampValidator.isTimeStampExtended(currentTimeStampToken,
                        getTimeStampValidityForTimeStampToken(currentTimeStampToken), timeStampExtension,
                        getTimeStampValidityForTimeStampToken(timeStampExtension))) {
                    setAllValidationInfos(documentAndTimeStampInfos,
                            new String[] { "L'estensione della marca temporale #" + (j + 1) + " contenuta nel file: "
                                    + timeStampExtensionFile + " (" + timeStampExtension.getTimeStampInfo().getGenTime()
                                    + ")" + " non estende la marca temporale #" + (i + 1) + " contenuta nel file: "
                                    + currentFile + " (" + currentTimeStampToken.getTimeStampInfo().getGenTime()
                                    + ")" },
                            null);
                    result = false;
                }
