...
/**
 * Definisce una classe che assume il ruolo di osservatore rispetto a una
 * {@link it.eng.crypto.ca.ICertificateAuthority Certification Authority}.
 *
 * @author Michele Rigo
 *
 */
public class CAObserver implements Observer {
     ...
     *
     * Alla notifica del cambiamento di stato dell'istanza osservata viene salvato nello storage il
     * certificato indicato. Inoltre viene recuperato il distribution point della CRL e istanziati 3
     * task che eseguono giornalmente lo scaricamento della crl a partire dal terzo giorno
     * precedente alla data di scadenza. La CRL attuale viene inoltre salvata nello Storage CRL .
     *
     * @param o   istanza della classe osservata ({@link it.eng.crypto.ca.ICertificateAuthority
     *            Certification Authority})
     * @param obj oggetto della chiamata notify ({@link java.security.cert.X509Certificate})
     */
	...
	synchronized (SINC) {
		...
		try {
			...
			if (oid.equals(X509Extension.cRLDistributionPoints)) {
			    log.debug("SubjectDN:"
				    + ((X509Certificate) obj).getIssuerX500Principal().getName());
			    CRLDistPoint p = CRLDistPoint.getInstance(extIn.readObject());
			    DistributionPoint[] points = p.getDistributionPoints();
			    for (int i = 0; i != points.length; i++) {
				String ret = StringUtils.remove(
					points[i].getDistributionPoint().getName().toString(),
					"GeneralNames:");
				...
				if (StringUtils.trim(ret).startsWith("6:")) {
				    ...
				    // Se il certificato è scaduto recupero ed inserico la CRL se
				    // non già presente
				    X509CRL crl = null;
				    try {
					crl = FactorySigner.getInstanceCRLStorage().retriveCRL(
						((X509Certificate) obj).getSubjectX500Principal()
							.getName(),
