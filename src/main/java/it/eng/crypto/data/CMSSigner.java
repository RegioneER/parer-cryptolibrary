...
/**
 * Implementa i controlli su firme di tipo CAdES. Il contenuto di un file è riconosciuto se
 * implementa le specifiche RFC5126
 *
...
public class CMSSigner extends P7MSigner {
	...
	if (timestamptokens == null) {
		...
		if (timestampTokensList.size() != 0) {
		    timestamptokens = timestampTokensList
			    .toArray(new TimeStampToken[timestampTokensList.size()]);
		}
	    } catch (Exception e) {
		e.printStackTrace();
	...
	if (signers == null || signers.isEmpty()) {
	    ...
	    // Controllo se l'algoritmo è di tipo SHA256 e che sia presente l'attributo contenente
	    // il certificato
		...
		if (signedTable == null) {
		    ...
		    complianceCheck.addWarning(
			    "Almeno una firma nella busta non presenta attributi firmati");
		    break;
		}
	...
	if (signature != null) {
		...
		validationInfos.addError(
			"Impossibile validare la marca poichè l'algoritmo di calcolo non è supportato: "
				+ hashAlgOID);
		validationInfos.setEsito(EsitoControllo.FORMATO_NON_CONOSCIUTO);
	    }
	...
	if (signers != null) {
	    for (SignerInformation signer : signers) {
		// Verifico la conformità con BouncyCastle chiamando il motodo che fa anche la
		...
		try {
		    ...
		    // in caso di errore di conformità l'eccezione non è di tipo
	...
	if (signers != null) {
		...
		if (attribute != null && attribute.getAttrValues() != null) {
		    TimeStampToken timestamptoken = null;
		    try {
			timestamptoken = new TimeStampToken(new CMSSignedData(attribute
				.getAttrValues().getObjectAt(0).toASN1Primitive().getEncoded()));
		    } catch (Exception e) {
			e.printStackTrace();
		    }
