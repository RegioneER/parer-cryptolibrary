...
/**
 * Classe di base per l'esecuzione delle operazioni di estrazione, analisi e validazione di firme
 * digitali.
 *
 * Fornisce già i metodi per il controllo della corrispondenza tra marche temporali e contenuto
 * firmato ({@link checkTimeStampTokenOverRequest}) e l'estrazione del contenuto della busta su file
 * ({@link getContentAsFile}). I metodi astratti da implementare sono i seguenti:
 * <ul>
 * <li>isSignedType: effettua l'istanziazione delle strutture di supporto e restituisce true se il
 * file analizzato è riconosciuto</li>
 ...
 * <li>canContentBeSigned: restituisce true se il contenuto sbustato può a sua volta essere firmato
 * (nel caso di firme multiple esterne)</li>
 * <li>getSignatures: recupera le firme contenute all'interno della busta. Ciascuna firma appartiene
 * ad una classe che implementa l'interfaccia ISignature: contiene al suo interno la lista delle
 * eventuali controfirme e il metodo di validazione (sia sul contenuto embedded che su file detached
 * - qualora previsto ).</li>
 * <li>validateTimeStampTokensEmbedded / validateTimeStampTokensDetached: implementano le
 * validazioni delle marche temporali interne o detached</li>
 * </ul>
 * Una classe estendente inoltre deve sovrascrivere uno dei metodi:
 * {@link validateTimeStampTokensEmbedded} o {@link validateTimeStampTokensDetached} a seconda se il
 * conenuto firmato sia all'interno della busta o esterno.
 *
...
public abstract class AbstractSigner {
    ...
    /**
     * Dimensione in byte oltre la quale effettuare una gestione NON in memoria (ie. in streaming)
     * del file Default: 20MB
     */
     ...
     *
     * @param content contenuto su cui eseguire il controllo
     *
     * @return true se il vi sono firme con formato riconosciuto
     */
    ...
    /**
     * Restituisce true se il contenuto sbustato può a sua volta essere firmato (nel caso di firme
     * multiple esterne)
     *
     * @return
     */
     ...
     */
    /**
     * Recupera l'hash del contenuto utilizzando l'algorimo di digest passato in ingresso - ogni
     * firma presente può utilizzare un proprio algoritmo di digest
     */
    ...
    /**
     * Recupera le CRL incluse all'interno del file firmato (solo per i formati di firma che lo
     * prevedono)
     */
    public abstract Collection<CRL> getEmbeddedCRLs();

    /**
     * Recupera i certificati inclusi all'interno del file firmato (solo per i formati di firma che
     * lo prevedono)
     */
    ...
    /**
     * Metodo di utilità che esegue la validazione di un timestamptoken rispetto al messaggio di
     * request contenente l'hashmap del contenuto da marcare.
     *
     * @param validationInfos struttura che raccoglie le informazioni riguardo all'esito della
     ...
     */
    protected void checkTimeStampTokenOverRequest(ValidationInfos validationInfos,
	    TimeStampToken timestamptoken, TimeStampRequest request) {
	...
	try {
	    ...
	    ContentInfo paramContentInfo = ContentInfo.getInstance(seq);
	    ...
	    checkTimeStampRequestOverTimeStampResponse(validationInfos, timestamptoken, request,
		    response);
	...
	/*
	 * Occorre quindi verificare che il timestamp sia stato effettivamente calcolato a partire
	 * dall'impronta del file in ingresso, cioè: SignerInfo.digestAlgorithm(ContentInfo.Content
	 * / ContentInfo.signedAttributes) =
	...
	try {
	    ...
	    // Refactored: Use Store and JcaX509CertificateConverter
	    ...
	    Collection<SignerInformation> signers = (Collection<SignerInformation>) cms
	    ...
	    /*
	     * Verifica che il certificato sia corretto ripetto al firmatario - la public key è
	     * correttamente associata al contenuto firmato
	     */
		...
		encodedDataToVerify = contentDigestAlgorithm
			.digest((byte[]) cms.getSignedContent().getContent());
	    }
	     ...
	     * poichè nelle m7m la chiamata restituisce un errore - occorre verificare che i due
	     * metodi restituiscano lo stesso oggetto (attualmente la mancata verifica viene
	     * segnalata solo come warning)
	     */
	    ...
	    byte[] tstInfoEncoded = contentDigestAlgorithm
		    .digest((byte[]) cms.getSignedContent().getContent());
		...
		try {
		    ...
		    boolean signatureVerified = Arrays.constantTimeAreEqual(encodedSignedData,
			    encodedDataToVerify);

		    log.debug("Verifica timestampToken: certificateVerified = "
			    + certificateVerified + ", signatureVerified=" + signatureVerified
			...
			validationInfos.addError(
				"La firma non è valida: l'hash di contenuto + attributi è "
		    ...
		    if (!contentVerified) {
			validationInfos.addWarning(
				"Il contenuto non corrisponde a quanto firmato: previsto "
					+ SignerUtil.asHex(tstInfoEncoded) + ", trovato "
					+ SignerUtil.asHex(contentDigest));
			validationInfos.setEsito(EsitoControllo.NEGATIVO);
		    }
		} catch (Exception e) {
		    validationInfos.addWarning(
			    "Errore durante la verifica del timestamp: " + e.getMessage());
		    validationInfos.setEsito(EsitoControllo.NEGATIVO);
		} finally {
	    ...
	    validationInfos.addError(
		    "Impossibile validare la marca poichè l'algoritmo di hashing non è supportato: "
			    + digestAlgorithmOID);
	    validationInfos.setEsito(EsitoControllo.FORMATO_NON_CONOSCIUTO);
	} catch (Exception e) {
	    validationInfos.addError(
     ...
     *
     *
     * @return restituisce il file originario se si tratta di marche detached, altrimenti salva il
     *         contenuto sbustato in un file temporaneo e ne restituisce il riferimento
     *
     * @throws IOException
     */
	...
	PEMParser pr = null;
	try {
	    pr = new PEMParser(new FileReader(file));
	...
	if (cmsSignedData == null) {
		...
		// Utilizzo il costruttore di CMSSignedData che accetta il ContentInfo così posso
		// impostare il limite
		...
		ContentInfo ci = ContentInfo.getInstance(
			new ASN1InputStream(new BufferedInputStream(stream), streamLength)
				.readObject());
		cmsSignedData = new CMSSignedData(ci);
	    } catch (Exception e) {
		if (stream != null) {
		...
		ContentInfo ci = ContentInfo.getInstance(
			new ASN1InputStream(new Base64InputStream(stream), streamLength)
				.readObject());
		try {
		    cmsSignedData = new CMSSignedData(ci);
