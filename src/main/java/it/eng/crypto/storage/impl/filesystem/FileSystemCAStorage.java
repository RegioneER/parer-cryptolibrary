    ...
    public List<X509Certificate> retriveActiveCA() throws CryptoStorageException {
        ...
        try {
            ...
            for (int i = 0; i < configs.size(); i++) {
                ...
                if (bean.isActive()) {
                    activeCertificates.add((X509Certificate) factorys
                            .generateCertificate(FileUtils.openInputStream(new File(bean.getFilePath()))));
                }
            }
        } catch (Exception e) {
        ...
        if (bean != null) {
            ...
            if (file.exists()) {
                try {
                    ...
                    log.error("Errore recupero certificato per X500Principal:" + subject.getName(), e);
